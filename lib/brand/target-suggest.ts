...
与えられた企業のブランド情報と「現状の主要顧客」をもとに、狙うべきターゲット顧客セグメントを論理的に提案します。

# 厳守事項
- 提案は3〜5案。各案は対象企業の事業・提供価値と整合し、実在しうる現実的な顧客像にすること。
- 「現状の主要顧客」を出発点とし、そこから派生・拡張する形でターゲット候補を提案すること（隣接領域・上位/下位セグメント・未開拓の近接ニーズ等）。
- ただし「現状の主要顧客」と名称・内容が重複する案は出さず、新規の切り口にすること。
- 各案について次を示すこと:
...
function buildUserMessage(brandInfo: Record<string, unknown>, existingNames: string[]): string {
  return [
    ...
    '',
    '# 現状の主要顧客（分析の出発点。ここから派生・拡張してターゲット候補を出す。同一の案は出さない）',
    existingNames.length > 0 ? existingNames.join('、') : '（なし）',
    '',
    '上記の現状の主要顧客から派生・拡張する形で、狙うべきターゲット顧客セグメントを3〜5案提案してください。',
  ].join('\n')
}
