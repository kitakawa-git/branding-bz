...
export const PATTERN_MEANINGS: Record<FunnelPattern, string> = {
  empathy_first:
    '共感が理解を上回っている。情緒的な支持は得ているが、それを支える言語と情報が足りていない状態。人が入れ替わると再現できず、社外にも伝播しない。共感を増やす施策は不要で、認知（情報供給）と理解（言語化）に投資を集中する。',
  behavior_first:
    '行動が共感を上回っている＝やらされ感の兆候。ルールを強めるのではなく、意味づけ・背景説明に投資する。',
  inward:
    '実践はしているが外や次の世代に渡せていない。反転点は越えているので、必要なのは動機づけではなく「渡すための言葉と機会」。',
  monotonic_decay:
