...
| 本番デプロイ | **済み** — 2026-07-16 まで反映。Vercel 自動デプロイ（git push で起動、コミット `d6ead6f`）。**⚠ 2026-07-18〜19 の未来設計 11コミットは未push＝本番未反映**（DBだけ先行適用済みの状態。コードを追いつかせるため早めの push 推奨） |
...
### 🆕 ブランドオントロジー未来設計（実装完了・未push）

- [ ] **push（＝共有・本番デプロイ）** — ローカルに11コミット。本番DBは先行適用済みなのでコードだけ遅れている状態。中身は superadmin 限定・追加のみ・検証済みでデプロイ自体は安全。並行セッションの事故で未push分を失うリスクがあるため早めに実施したい。
- [ ] **実機E2E** — superadmin でログインし、テックブリッジ（デモ）で一周してから ID INC. へ。獲得目標を作る → 実績に測定値を入れる → `verifies` で繋ぐ → 進捗が出る → 提供価値を `target` にする → コピーAIで「目指す」として出る（事実として断定されない）ことを確認。※ここまでの検証は tsc/build・ユニット・SQL（ロールバック付き）・Node からのブロック組み立てと生成実行のみで、**ブラウザ画面での操作確認は未実施**。
