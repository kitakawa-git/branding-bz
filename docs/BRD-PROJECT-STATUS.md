...
| 本番デプロイ | **済み** — 2026-07-21 まで反映（Vercel 自動デプロイ・`b5ea497` でオントロジーカード刷新一式が Ready）。未来設計の11コミットも 2026-07-19 に push 済みで、コードとDBは揃っている |
...
| 未コミットWIP | **なし**（2026-07-16 時点で working tree は本ファイル更新のみ。実装コード側はすべて `d6ead6f` までコミット・push 済み。stash も 0件＝古い RLS cleanup 系 WIP は 2026-07-16 のセッションで内容確認の上 drop 済み〔react-markdown 依存追加は既に `ad6d93a` で main 反映済み・残る3ファイルの UI 改善は判断で不採用〕） |
