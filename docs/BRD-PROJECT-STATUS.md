...
| 本番デプロイ | **済み** — 2026-07-16 まで反映。Vercel 自動デプロイ（git push で起動、コミット `d6ead6f`） |
| DB マイグレーション（本セッションで適用） | **今回（2026-07-16）はDDLなし**（アプリ層のみ）。※2026-06-30 適用分（既記載）: `blocked_competitor_domains` / `companies.approval_status` / `companies.competitor_flag`。※2026-07-14 その5分: `companies.representative_profile TEXT` も引き続き有効 |
