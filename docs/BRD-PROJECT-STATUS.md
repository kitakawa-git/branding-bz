...
**最終更新:** 2026-07-14
**更新者:** Claude Code（2026-07-14 その3：会社情報の競合カラードットの未指定フォールバック/空非表示、ポータルに会社概要ページ `/portal/about` 新設→ユーザーメニュー内配置＋名称を「私たちについて」に統一＋アイコンを Users に、事業内容の編集を「基本情報」へ・ポータル表示を「私たちについて」へ移動（共通コンポ `BusinessContentEditor`）、基本情報「設立」を年/月ドロップダウン化＋沿革と共通ヘルパー `lib/year-month.ts` 化、基本情報から「競合企業・サービス」欄と「所在地」欄をUI削除（列/連携は温存）、基本情報の「概要」を業種の上へ移動。DB追加DDLなし。最新コミット `4344e3d`）
