...
| 本番デプロイ | ⚠ **一部未反映** — 2026-07-21（その2）の途中まで。`fb5d027`（表現ルールの区分5→3）以降 4コミットが未push で、**このマイグレーションだけ本番DBに適用済み＝本番のスーパー管理では廃止済み区分が選択肢に残り、保存するとCHECK違反になる**。早めの push が必要。以下は従来の記録：（Vercel 自動デプロイ・最新 `2dc2522`）。オントロジーカード刷新一式 (`b5ea497`) 以降、superadminオントロジーウィザードのUI微調整・言葉のルールUI整合・診断ツールの表現ルール表示・その他個別 fix (◯×文字化け・admin/members ダーク revert・BrandMap3D 白背景) が21件 push 済み。未来設計の11コミットも 2026-07-19 に push 済みで、コードとDBは揃っている |
...
| 未コミットWIP | **2件**（本セッション作業・北川さんの明示指示待ちのため未 push）: (a) `app/superadmin/companies/[id]/_sections/OntologySummaryHub.tsx` — オントロジーカードのプレゼンモード（フルスクリーン）入り/出しに opacity+scale の transition アニメ追加（`presentEntering` state・`requestAnimationFrame` 起動・close は `closePresent()` で300ms待って アンマウント）＋3つのamber警告ボタン（理念未登録／提供価値未登録／未接続N件）を「押せる」と分かる強化スタイルに統一（border+shadow+hover-translate+ArrowRight icon+underline+focus-visible ring）。(b) `lib/brand/rule-display.ts` — 別セッション作業（本セッションでは触っていない・personality の表現ルール表示関連の続き）。stash 0件 |
...
- [ ] **実機E2E（最重要の残タスク）** — superadmin でログインし、テックブリッジ（デモ）で一周してから ID INC. へ。
  ...
  - ※**これらは全て画面での操作確認が未実施**。ローカルプレビューが未ログインで `/superadmin` に到達できず、担保できているのは tsc/build・ユニット・SQL（ロールバック付き）・Nodeからのロジック実行まで。特に**3Dの自動フィット係数**（`VIEW_W*0.40` / `VIEW_H*0.38`）と**ハイライトの効き具合**（`MIN_LEN=4`）は実物を見ないと詰められない。
