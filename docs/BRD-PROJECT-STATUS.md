...
| 本番デプロイ | **済み** — 2026-07-21（その2）まで反映（Vercel 自動デプロイ・最新 `2dc2522`）。オントロジーカード刷新一式 (`b5ea497`) 以降、superadminオントロジーウィザードのUI微調整・言葉のルールUI整合・診断ツールの表現ルール表示・その他個別 fix (◯×文字化け・admin/members ダーク revert・BrandMap3D 白背景) が21件 push 済み。未来設計の11コミットも 2026-07-19 に push 済みで、コードとDBは揃っている |
