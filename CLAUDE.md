...
- コンテンツ/セクションカード内パディング: **`p-4 sm:p-5`**（スマホ16px / PC20px）＝ポータル標準。`p-4` 単独は高密度/リスト/ウィジェット、`p-3` は密な投稿カード（ダッシュボードの DashboardPostCard 等）。`p-5 pb-3` 等の下余白独自指定カードは据え置き
...
**色は「役割で1色」。** 同じ役割に別系統の色を混ぜない。新規実装時は下表のトークンだけを使う。

#### Layer 0｜ニュートラル（無彩） = **gray**
`slate / neutral / zinc / stone は使わない`（すべて gray）。

| トークン | 用途 |
|---------|------|
| `text-gray-900` | 見出し、ブランド名（メイン文字色） |
| `text-gray-700` | サブテキスト、説明文（大）・ラベル |
| `text-gray-600` | 説明文（小）、フッターリンク、カード本文 |
| `text-gray-500` | タグライン、補足 |
| `text-gray-400` | コピーライト（最も薄い文字） |
| `bg-white` | メイン背景、フッター、カード地 |
| `bg-gray-50` | セクション背景（交互）、淡い面 |
| `bg-gray-100` | 区切り面・ホバー地 |
| `border-gray-200` | 標準ボーダー |

#### Layer 1｜アクセント／主アクション ＋ 情報 = **blue**
アクセント（主アクション・選択状態・バッジ）と情報（callout）はいずれも **blue**。`indigo / sky は使わない`（blueへ）。

| 用途 | クラス |
|------|--------|
| 主ボタン・選択/アクティブ状態・番号バッジ・進捗 | `bg-blue-600 hover:bg-blue-700 text-white` |
| リンク・インライン文字・アクセント文字 | `text-blue-600` / `text-blue-700`（バッジ）、リンクは `hover:underline` |
| info callout・補助強調の地 | `bg-blue-50` / `bg-blue-100` ＋ `text-blue-700` |
| ボーダー・選択枠 | `border-blue-200`（淡）/ `border-blue-500`（選択） |
| フォーカスリング | `focus:ring-blue-500` |

> マーケLPの**主CTAは黒のまま**（既存グラスモーフィズム）。blueはバッジ・リンク・ハイライトのアクセント。LPのアクセントバッジは地 `rgba(0, 97, 255, 0.1)` ＋ `text-blue-700`。

#### Layer 2｜意味色（セマンティック・各1系統）
| 役割 | canonical | 文字 | 地 | ボーダー | 禁止（統合済み） |
|------|-----------|------|----|---------|----------------|
| 成功 success | **green** | `text-green-700` | `bg-green-50` | `border-green-200` | emerald |
| 警告 warning | **amber** | `text-amber-700` | `bg-amber-50` | `border-amber-200` | yellow, orange |
| エラー error | **red** | `text-red-700` | `bg-red-50` | `border-red-200` | rose（※カテゴリ用途を除く） |

#### Layer 3｜カテゴリ／チャート用パレット（多色は意図的・統合しない）
KPIカテゴリ・ペルソナのステージ・ジャーニーマップ・スコア/ランクのティア配色（例 green→blue→amber→orange→red）・感情スケール（red→orange→gray→green→emerald）等、「**項目や段階を区別する**」ための多色。ここの色は意味色ではないので Layer 2 のルールを適用しない（emerald/orange/rose もここでは正当）。`violet は使わず purple に寄せる`。

#### Layer 4｜ブランド空気感（マーケLP装飾のみ）
Hero/CTA背景のパステル・オーロラ階調（ラベンダー `#c4b5fd` / アンバー `#fdba74` / ミント `#a7f3d8` / ピンク `#fbcfe8`）は温存。影アクセント `rgba(12, 74, 110, …)` も据え置き。→ 詳細は「グラデーション背景」節。

#### 意図的な例外（リファクタ対象外）
- **superadmin の紺テーマ** `#1e3a5f`（内部コンソールの識別）
- **顧客カラーの初期値** `#1a1a1a`（UIトークンでなくデータのデフォルト）
- **admin/login/callback のダークグラデ** `#0f0f1a / #141425 / #0d0d1a`
