...
// 親 (page.tsx) が structure を保持。表示は props 由来、変更は API→onChanged() で再取得
...
function EditableName({ value, onSave, className }: { value: string; onSave: (v: string) => void; className?: string }) {
  ...
  if (editing) {
    return (
      <form className="flex-1 flex items-center gap-1" onSubmit={(e) => { e.preventDefault(); if (draft.trim()) { onSave(draft.trim()); setEditing(false) } }}>
