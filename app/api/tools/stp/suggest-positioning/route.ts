...
出力JSONスキーマ:
{
  ...
  "items": [
    { "name": "自社", "x": 60, "y": 65, "color": "#3B82F6", "is_self": true, "traits": "自社の強み（ターゲットに対して活かせる強み。2〜3文）", "reasoning": "なぜこの座標か（1文）", "confidence": "high" },
    { "name": "競合A", "x": 30, "y": 60, "color": "#EF4444", "is_self": false, "traits": "", "reasoning": "なぜこの座標か（1文）", "confidence": "medium" }
  ]
}
...
- 自社（is_self: true）の traits は必須。ユーザー入力の「自社の強み」があればそれを踏まえて磨き上げ、なければメインターゲット・購買決定要因を踏まえて新規に2〜3文で提案すること
- 競合（is_self: false）の traits は、ユーザーが既に入力している場合のみそのまま引き継ぐこと。未入力ならAIが代わりに書かず、空文字（""）のままにする（ユーザーが自分で判断して入力する項目のため）
- confidence の判定基準:
  - "high": ユーザー入力（自社の強み / 競合分析の特徴）に座標を裏付ける具体的な記述がある
  - "medium": 一部の記述から推測できる
