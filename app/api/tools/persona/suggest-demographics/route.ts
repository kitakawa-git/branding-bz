...
重要: 1人の具体的な個人（実名・具体的な住所・家族構成など）を作り込むのではなく、役割名・年齢層・短い特徴で、ターゲットセグメントを簡潔に描いてください。個人固有の肉声（口癖・1日の過ごし方など）は無理に作らず、セグメントとして個別性が無い項目は空文字で構いません。
...
出力JSONスキーマ:
{
  ...
  "location": "居住地（具体住所でなく型レベル。例: 地方都市 / 都市部）",
  "annual_income": "（セグメントで個別性が無ければ空文字でよい）",
  "family": "（セグメントで個別性が無ければ空文字でよい）",
  "education": "（セグメントで個別性が無ければ空文字でよい）",
  "hobbies": ["代表傾向1", "代表傾向2"],
  "media_channels": ["情報収集の代表傾向1", "情報収集の代表傾向2"],
  "personality_traits": ["代表的な傾向1", "代表的な傾向2", "代表的な傾向3"],
  "daily_routine": "（セグメントで個別性が無ければ空文字でよい）",
  "quote": "（セグメントで個別性が無ければ空文字でよい）"
}`
