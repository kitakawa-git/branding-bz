// ラーニング テーマ 並び替えAPI（管理者・service_role 経由）
// PATCH /api/learning/themes/reorder  body: [{ id, sort_order }]（{ orders: [...] } 形式も許容）
// カテゴリー内の並び替え想定だが、各テーマの sort_order を個別更新するだけ（カテゴリー跨ぎは PATCH [id] で）
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    console.error('[Learning Themes Reorder] エラー:', err)
