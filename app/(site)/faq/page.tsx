...
const faqCategories = [
    ...
    items: [
      {
        ...
        a: 'branding.bz は、中小企業のブランドを「構築→浸透→発信」まで一貫支援するSaaSです。AIを活用した構築ツール（ブランドカラー定義・STP分析等）、社内浸透プラットフォーム（ブランド掲示・Good Action投稿・KPI・目標管理・インナーサーベイ等）、スマート名刺による社外発信、そしてブランドスコアによる浸透度の定量化までをワンストップで提供します。',
      },
      {
        q: 'どのような企業に向いていますか？',
  ...
  },
  {
    ...
    items: [
      ...
      {
        q: 'プランの違いは何ですか？',
        a: 'Free（¥0）はAI構築ツールの体験とブランド掲示の編集、Standard（¥19,800/月）はAI構築ツール使用無制限＋Good Action投稿・お知らせ配信＋スマート名刺による発信、Premium（¥59,800/月）はビデオラーニング・ブランド理解度テスト・KPI・目標管理・ブランドスコア（簡易版）など社内浸透の全機能をご利用いただけます。インナーサーベイと統合ブランドスコアによる計測は、結果の解釈と打ち手をセットでご提供するため Enterprise（個別見積）でのご提供です。詳しくは料金ページをご確認ください。',
      },
      {
